...
jQuery(document).ready(function ($) {
  ...
  $('.fadeIn').each(function (i, el) {
    // find all the .fadeIn elements. loop through them so we're dealing with one at a time
    ...
    TweenMax.set(el, {
      opacity: 0
    }); // use tweenmax to instantly set opacity to 0

    $(el).waypoint({
      // make a waypoint on this element
      handler: function handler(direction) {
        // waypoint handler function 
        console.log('waypoint hit!');
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  burgerTimeline.from(".about", 0.4, {
    delay: 1,
    opacity: 0
  });
  burgerTimeline.from(".work", 0.5, {
    opacity: 0
  });
  burgerTimeline.from(".resume", 0.5, {
    opacity: 0
  });
  burgerTimeline.from(".contact", 0.5, {
    opacity: 0
